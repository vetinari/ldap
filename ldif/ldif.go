// Package ldif contains an  LDIF parser (RFC 2849).
//
...
func (l *LDIF) parseEntry(lines []string) (entry *Entry, err error) {
	...
	case "moddn", "modrdn":
		...
		/*
			...
			// FIXME: controls for ModifyDN
			return &Entry{
				ModifyDN: &ldap.NewModifyDNRequest(dn, newRDN, delOld, newSuperior),
			}, nil
		*/
	default:
...
func (l *LDIF) parseControls(lines []string) ([]ldap.Control, []string, error) {
	...
	for {
		...
		if ctrlValue == "" {
			...
			// convert ctrlValue to *ber.Packet and decode with
